...
def resume_data():
    return {
        ...
        "experience_years": 3,
        "experience": [
            {"role": "Backend Engineer", "company": "TechCorp", "highlights": ["Built APIs"]}
        ],
...
class TestHappyPath:
    ...
    async def test_handles_markdown_wrapped_response(self, agent, resume_data, jd_data):
        agent.chain = FakeChain(response=FakeMsg('''```json
...
class TestListCoercion:
    @pytest.mark.asyncio
    async def test_coerces_non_list_keywords_to_empty(self, agent, resume_data, jd_data):
...
class TestFallback:
    ...
    @pytest.mark.asyncio
    async def test_malformed_llm_response_returns_fallback(self, agent, resume_data, jd_data):
...
class TestInputTrimming:
    ...
    async def test_trims_experience_to_top_5(self, agent, jd_data):
        ...
        big_resume = {
            ...
            "experience_years": 5,
            "experience": [
                {"role": f"Role {i}", "company": f"Co {i}", "highlights": ["A", "B", "C", "D"]}
                for i in range(10)
            ],
            "education": [],
            "projects": [],
        }
        fake = FakeChain(response=FakeMsg('{"ats_score": 50, "matching_keywords": [], "missing_keywords": [], "formatting_issues": [], "recommendations": [], "summary": ""}'))
        agent.chain = fake
